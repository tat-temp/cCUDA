...
)
{
    ...
    // REGISTER DISCIPLINE, paired with the 2-wide hash (see CUDAHash.cuh). Neither the running
    // scalar S[4] nor the running remainder rem[4] is kept in registers any more. Together they
    // cost 16 registers across the whole batch loop while being read almost nowhere -- S only at
    // the four found sites and at exit, rem only in the loop condition and at exit -- and holding
    // them is what pushed this kernel past its 128-register ceiling once px3A had to stay live
    // across block2 (measured before this change: 128 registers, 48/44 bytes of spill).
    //
